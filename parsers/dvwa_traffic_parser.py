"""Parser for DVWA traffic snapshots."""
...
def _label_from_record(record: dict[str, Any]) -> int:
    ...
    return 1 if ("attack" in scenario or category not in {"", "normal", "benign"}) else 0
...
def parse_dvwa_traffic(dataset_dir: str | Path, max_rows: int = 250_000) -> list[dict]:
    """Parse DVWA JSONL traffic snapshots."""
    root = Path(dataset_dir)
    if not root.exists():
        raise FileNotFoundError(f"DVWA traffic path not found: {root}")

    files = sorted(path for path in root.rglob("*.jsonl"))
    records: list[dict] = []
    if not files:
        logger.warning(f"DVWA traffic path has no JSONL files: {root}")
        return records
    ...
    logger.info(f"DVWA traffic parsed records: {len(records)}")
